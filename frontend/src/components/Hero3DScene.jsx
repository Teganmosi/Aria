...
import { Float, OrbitControls, MeshDistortMaterial, Stars, Sparkles, Icosahedron, Torus, Ring } from '@react-three/drei'
  ...
  return (
    <Float speed={1.2} rotationIntensity={0.2} floatIntensity={0.4}>
      <group ref={groupRef} position={[-3, 0, -1]} scale={0.6}>
        ...
        {/* Connection lines */}
        {[0, 1, 2, 3].map((i) => (
          <mesh key={`line-${i}`} rotation={[0, i * Math.PI / 2, 0]}>
      ...
      <group ref={groupRef} position={[-1.2, 1.2, -1.2]} scale={0.5}>
        {nodes.map((pos, i) => (
          <mesh key={i} position={pos}>
            ...
            <meshStandardMaterial
              color={i === 0 ? "#4f46e5" : "#818cf8"}
              emissive={i === 0 ? "#4f46e5" : "#818cf8"}
        ...
        {nodes.slice(1).map((pos, i) => (
          ...
          >
            <cylinderGeometry args={[0.015, 0.015, Math.sqrt(pos[0]**2 + pos[1]**2 + pos[2]**2), 8]} />
